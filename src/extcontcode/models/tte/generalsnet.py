...
class GeneralSNetTTE(nn.Module):
    """
    ...
    def __init__(
        ...
        g_x_z0=nn.Sequential(),
        g_x_z01=nn.Sequential(),
        g_x_z1=nn.Sequential(),
        g_x_za=nn.Sequential(),
        g_x_zt=nn.Sequential(),
    ...
    def ortho_reg(self):
        ...
        w_z0 = _get_absolute_rowsums(self.g_x_z0[0].weight)
        w_z1 = _get_absolute_rowsums(self.g_x_z1[0].weight)
        w_z01 = _get_absolute_rowsums(self.g_x_z01[0].weight)
        w_za = _get_absolute_rowsums(self.g_x_za[0].weight)
        w_zt = _get_absolute_rowsums(self.g_x_zt[0].weight)
    ...
    def get_py_tz(self, t, y, dmy, z):
        ...
        assert self.dim_z == 6
        z0, z01, z1, za, zt = z[:, :1], z[:, 1:2], z[:, 2:3], z[:, 3:5], z[:, 5:]
    ...
    def predict(self, x):
        ...
        y0 = self.f_z_y0(th.cat((z0, z01, za), 1))
        y1 = self.f_z_y1(th.cat((z1, z01, za), 1))
    ...
    def get_z(self, x):
        ...
        za = self.g_x_za(x)
        zt = self.g_x_zt(x)
        m_z = th.cat((z0[:, :1], z01[:, :1], z1[:, :1], za[:, :2], zt[:, :1]), 1)
        lvar_z = th.cat((z0[:, 1:], z01[:, 1:], z1[:, 1:], za[:, 2:], zt[:, 1:]), 1)
        return m_z, lvar_z
    ...
    def update_stdy(self, loader, prior=False):
        ...
        # TODO: switch to separate stds per treatment t
    ...
    def estimate_elbo(self, t, xm, y, n_data, return_all=False, pop=None):
        ...
        if return_all:
            ...
            return (
                data_fit + log_pt_z - kl_u - self.beta * kl_z - self.ortho_reg()
            ) / n_train
...
class IdentSNetTTE(GeneralSNetTTE):
    def __init__(
